package com.wangzk.im.websocket;
 ...
 * @Title: WebSocket
 * @ProjectName: im
 * @PackageName: com.wangzk.im.websocket
 * @Description: TODO
 * @author: wangzk
...
@Log4j2
@Component
@ServerEndpoint("/im/{username}/{token}")
public class WebSocket {
     ...
     */
    @OnOpen
    public void onOpen(@PathParam("username") String username, @PathParam("token") String token, Session session) throws Exception{
        //校验token
        if (!jwtTokenUtil.validateToken(token,username)){
            throw new Exception("IM校验token失败，连接失败");
        }
        //判断该用户是否已经登录
